  ...
  const mutation = useMutation({
    mutationFn: async (formData: FormValues) => {
      ...
      // Otherwise create a new project
        ...
        const {
          data,
          error
        } = await supabase.from("projects").insert(newProject).select();
        if (error) {
    ...
    onSuccess: () => {
      ...
      toast({
        ...
        description: isEditing ? "The project has been updated successfully." : "New project has been created successfully."
      });
      onClose();
    },
    onError: error => {
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            field
                  ...
                  <FormControl>
                    <CustomerSelector selectedCustomerId={field.value} onSelectCustomer={customerId => {
                console.log('Customer selected:', customerId);
                field.onChange(customerId);
              }} disabled={mutation.isPending} preventClose={true} />
                  </FormControl>
                  <FormDescription>
            ...
            <FormField control={form.control} name="budget_hours" render={({
            field
          }) => <FormItem>
                  <FormLabel>Budget (Hours)</FormLabel>
                  <FormControl>
                    <Input type="number" min="0" step="0.5" required {...field} onChange={e => field.onChange(parseFloat(e.target.value) || 0)} />
                  </FormControl>
                  <FormDescription>
