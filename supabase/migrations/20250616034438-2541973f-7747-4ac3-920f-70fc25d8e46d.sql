...
-- Fix the log_update_audit function to resolve the FULL JOIN issue
...
BEGIN
  ...
  IF TG_TABLE_NAME = 'timesheet_entries' THEN
    -- Get project or contract name for context - fixed FULL JOIN issue
    ...
    description_val := 'Updated timesheet entry for ' || COALESCE(entity_name_val, 'Unknown Project/Contract') || 
